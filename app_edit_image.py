...
# Standalone edit image (no Gradio UI)
def standalone_edit_image(
    ...
    """
    Run the edit_image function in standalone mode (no Gradio UI).
    Args:
        image_path (str): Path to the input image.
        prompt (str): Edit prompt.
        show_thinking (bool): Whether to show thinking process.
    ...
    Returns:
        (PIL.Image.Image, str): Edited image and thinking process (if any).
    """
    image = load_example_image(image_path)
    if image is None:
        print("Failed to load image:", image_path)
        return None, ""
    result_image, thinking = edit_image(
        image, prompt, show_thinking, cfg_text_scale, cfg_img_scale,
    ...
    return result_image, thinking


if __name__ == "__main__":
    # # Example usage:
    # result_img, thinking = standalone_edit_image(
    #     image_path='test_images/women.jpg',
    #     prompt="She boards a modern subway, quietly reading a folded newspaper, wearing the same clothes.",
    #     show_thinking=True,
    #     output_path='test_images/women_edited.jpg',
    #     seed=20,
    #     num_timesteps=30,
    # )
    # exit(0)
    ...
    FILE_DONE = DIR_OUTPUT/"done.txt"

    # if input doesn't exist, exit
    if not DIR_INPUT.exists():
        print(f"Input directory {DIR_INPUT} does not exist. Please create it and add images.")
        exit(1)
    ...
    prompts = {
        "cap": "Add a realistic cap of a random style and a random color in each run (e.g., baseball cap, beanie, flat cap, bucket hat, snapback, etc.) to the cropped face image. The cap must align naturally with the head orientation and angle in the input image, not with the cap’s default orientation. Ensure correct alignment, lighting, shadows, textures, and proportions so that the cap looks naturally worn. Both the style and the color of the cap should vary across runs so that multiple generated images feature different realistic types and colors of caps. It is acceptable if parts of the cap (e.g., the visor of a baseball cap) are cropped due to the face crop. The input image must remain unchanged in resolution, dimensions, and framing — do not resize, pad, crop, zoom, or alter the background. Do not alter the person’s facial features, hairstyle, skin tone, or background. The output must be strictly photorealistic with pixel dimensions identical to the input. No borders, no distortions, and no extra accessories.",
        "sunglass": "Add a pair of realistic sunglasses to the cropped face image. The sunglasses must fit naturally on the face with correct alignment, lighting, reflections, shadows, and proportions, following the head orientation in the input image. Each generated image should feature a different realistic style of sunglasses (e.g., aviator, wayfarer, round, sport, rectangular, etc.) with a randomly applied realistic glass tint (e.g., black, brown, mirrored, gradient) while keeping all other details unchanged. The input image must remain identical in resolution, dimensions, and framing — no resizing, no upscaling, no downscaling, no padding, no zooming, no cropping, and no changes to the face orientation. The sunglasses may be partially visible if they extend beyond the crop. Do not alter the person’s facial features, hairstyle, skin tone, or background. The output must be strictly photorealistic with pixel dimensions identical to the input. No borders, no distortions, and no extra accessories.",
    }
    
    # images to process, resume
    images_to_do = set([*map(lambda x: x.name, DIR_INPUT.glob("*"))])
    if not FILE_DONE.exists():
        open(FILE_DONE, "w").close()
    with open(FILE_DONE, "r") as f:
        images_done =  set([line for line in f.read().splitlines() if line.strip() != ""])
    images_todo = images_to_do - images_done
    images_todo = sorted(list(map(lambda x: Path(x), images_todo)))
    # print("images:", images_todo)

    for img_path in images_todo:
        for key, prompt in prompts.items():
            input_path = DIR_INPUT / img_path
            output_path = DIR_OUTPUT / f"{img_path.stem}_{key}.jpg"
            result_img, thinking = standalone_edit_image(
